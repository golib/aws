...
var (
	...
	Session = session.Must(session.NewSession(&internal.Config{
		Endpoint:   internal.String(server.URL[7:]),
		DisableSSL: internal.Bool(true),
	}))
)

// NewMockClient creates and initializes a client that will connect to the mock server
func NewMockClient(cfgs ...*internal.Config) *client.Client {
