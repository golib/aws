...
func New(cfgs ...*internal.Config) *Session {
...
func NewSession(cfgs ...*internal.Config) (*Session, error) {
	envCfg := loadEnvConfig()
...
//     sess, err := session.NewSessionWithOptions(session.Options{
//          Config: internal.Config{Region: internal.String("us-east-1")},
//          Profile: "profile_name",
//     })
//
...
func oldNewSession(cfgs ...*internal.Config) *Session {
	cfg := defaults.Config()
...
func newSession(envCfg envConfig, cfgs ...*internal.Config) (*Session, error) {
	cfg := defaults.Config()
	handlers := defaults.Handlers()

	// Get a merged version of the user provided config to determine if
	// credentials were.
	userCfg := &internal.Config{}
...
func mergeConfigSrcs(cfg, userCfg *internal.Config, envCfg envConfig, sharedCfg sharedConfig, handlers request.Handlers) {
	// Merge in user provided configuration
	cfg.MergeIn(userCfg)

	// Region if not already set by user
	if len(internal.StringValue(cfg.Region)) == 0 {
	...
	if cfg.Credentials == credentials.AnonymousCredentials && userCfg.Credentials == nil {
		if len(envCfg.Creds.AccessKeyID) > 0 {
			...
			cfg.Credentials = credentials.NewCredentials(&credentials.ChainProvider{
				VerboseErrors: internal.BoolValue(cfg.CredentialsChainVerboseErrors),
...
func initHandlers(s *Session) {
	...
	if !internal.BoolValue(s.Config.DisableParamValidation) {
...
//     sess.Copy(&internal.Config{Region: internal.String("us-west-2")})
func (s *Session) Copy(cfgs ...*internal.Config) *Session {
	newSession := &Session{
...
func (s *Session) ClientConfig(serviceName string, cfgs ...*internal.Config) client.Config {
	s = s.Copy(cfgs...)
	endpoint, signingRegion := endpoints.NormalizeEndpoint(
		internal.StringValue(s.Config.Endpoint),
		serviceName,
		internal.StringValue(s.Config.Region),
		internal.BoolValue(s.Config.DisableSSL),
		internal.BoolValue(s.Config.UseDualStack),
	)
