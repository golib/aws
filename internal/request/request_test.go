...
func TestRequestRecoverRetry5xx(t *testing.T) {
	...
	s := awstesting.NewClient(internal.NewConfig().WithMaxRetries(10))
...
func TestRequestExhaustRetries(t *testing.T) {
	...
	s := awstesting.NewClient(internal.NewConfig().WithSleepDelay(sleepDelay))
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
	s.Handlers.Send.Clear() // mock sending
...
func TestRequestRecoverExpiredCreds(t *testing.T) {
	...
	s := awstesting.NewClient(&internal.Config{MaxRetries: internal.Int(10), Credentials: credentials.NewStaticCredentials("AKID", "SECRET", "")})
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
...
func TestRequestUserAgent(t *testing.T) {
	s := awstesting.NewClient(&internal.Config{Region: internal.String("us-east-1")})
	//	s.Handlers.Validate.Clear()
	...
	expectUA := fmt.Sprintf("foo/bar %s/%s (%s; %s; %s)",
		internal.SDKName, internal.SDKVersion, runtime.Version(), runtime.GOOS, runtime.GOARCH)
...
func TestRequestRecoverTimeoutWithNilBody(t *testing.T) {
	...
	s.Handlers.AfterRetry.PushBack(func(r *request.Request) {
		if r.Error != nil {
			r.Error = nil
			r.Retryable = internal.Bool(true)
			r.RetryCount++
		}
	})
	s.Handlers.Send.Clear() // mock sending
