...
func New(cfg internal.Config, clientInfo metadata.ClientInfo, handlers Handlers, retryer Retryer,
...
func (r *Request) WillRetry() bool {
	return r.Error != nil && internal.BoolValue(r.Retryable) && r.RetryCount < r.MaxRetries()
}
...
func debugLogReqError(r *Request, stage string, retrying bool, err error) {
	if !r.Config.LogLevel.Matches(internal.LogDebugWithRequestErrors) {
		return
	}
...
func (r *Request) Send() error {
	for {
		if internal.BoolValue(r.Retryable) {
			if r.Config.LogLevel.Matches(internal.LogDebugWithRequestRetries) {
