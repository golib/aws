...
import (
	...
	"github.com/golib/aws/service/awstesting"
...
func buildRequest(serviceName, region, body string) (*http.Request, io.ReadSeeker) {
	...
	req.Header.Add("X-aws-Target", "prefix.Operation")
	req.Header.Add("Content-Type", "application/x-aws-json-1.0")
	req.Header.Add("Content-Length", string(len(body)))
	req.Header.Add("X-aws-Meta-Other-Header", "some-value=!@#$%^&* (+)")
	req.Header.Add("X-aws-Meta-Other-Header_With_Underscore", "some-value=!@#$%^&* (+)")
	req.Header.Add("X-aws-Meta-Other-Header_With_Underscore", "some-value=!@#$%^&* (+)")
	return req, reader
}
...
func TestPresignRequest(t *testing.T) {
	...
	expectedHeaders := "content-length;content-type;host;x-aws-meta-other-header;x-aws-meta-other-header_with_underscore"
	expectedSig := "d1f24db65bbe23793498a7a9e20a57efdccbeed230b20ec812eab0c95edad2c8"
	expectedCred := "AKID/19700101/us-east-1/dynamodb/aws4_request"
	expectedTarget := "prefix.Operation"

	q := req.URL.Query()
	assert.Equal(t, expectedSig, q.Get("X-Aws-Signature"))
	assert.Equal(t, expectedCred, q.Get("X-Aws-Credential"))
	assert.Equal(t, expectedHeaders, q.Get("X-Aws-SignedHeaders"))
	assert.Equal(t, expectedDate, q.Get("X-Aws-Date"))
	assert.Empty(t, q.Get("X-Aws-Meta-Other-Header"))
	assert.Equal(t, expectedTarget, q.Get("X-Aws-Target"))
}
...
func TestSignRequest(t *testing.T) {
	...
	expectedDate := "19700101T000000Z"
	expectedSig := "AWS4-HMAC-SHA256 Credential=AKID/19700101/us-east-1/dynamodb/aws4_request, SignedHeaders=content-length;content-type;host;x-aws-date;x-aws-meta-other-header;x-aws-meta-other-header_with_underscore;x-aws-security-token;x-aws-target, Signature=e52434f6d95c8f34cfb43fe8f073e9ab1f57a7148b0b14ab063e45a767b644bd"
...
func TestSignBodyS3(t *testing.T) {
	...
	hash := req.Header.Get("X-Aws-Content-Sha256")
...
func TestSignPrecomputedBodyChecksum(t *testing.T) {
	...
	req.Header.Set("X-Aws-Content-Sha256", "PRECOMPUTED")
	signer := buildSigner()
...
func TestAnonymousCredentials(t *testing.T) {
	svc := awstesting.NewClient(&service.Config{Credentials: credentials.AnonymousCredentials})
	r := svc.NewRequest(
		&request.Operation{
			Name:       "BatchGetItem",
			HTTPMethod: "POST",
	...
	assert.Empty(t, urlQ.Get("X-Aws-Signature"))
	assert.Empty(t, urlQ.Get("X-Aws-Credential"))
	assert.Empty(t, urlQ.Get("X-Aws-SignedHeaders"))
	assert.Empty(t, urlQ.Get("X-Aws-Date"))

	hQ := r.HTTPRequest.Header
	assert.Empty(t, hQ.Get("Authorization"))
	assert.Empty(t, hQ.Get("X-Aws-Date"))
}

func TestIgnoreResignRequestWithValidCreds(t *testing.T) {
	svc := awstesting.NewClient(&service.Config{
		Credentials: credentials.NewStaticCredentials("AKID", "SECRET", "SESSION"),
		Region:      service.String("us-west-2"),
	})
	r := svc.NewRequest(
		&request.Operation{
			Name:       "BatchGetItem",
...
func TestIgnorePreResignRequestWithValidCreds(t *testing.T) {
	...
	SignSDKRequest(r)
	sig := r.HTTPRequest.Header.Get("X-Aws-Signature")

	SignSDKRequest(r)
	assert.Equal(t, sig, r.HTTPRequest.Header.Get("X-Aws-Signature"))
}

func TestResignRequestExpiredCreds(t *testing.T) {
	creds := credentials.NewStaticCredentials("AKID", "SECRET", "SESSION")
	svc := awstesting.NewClient(&service.Config{Credentials: creds})
	r := svc.NewRequest(
		&request.Operation{
			Name:       "BatchGetItem",
			HTTPMethod: "POST",
...
func TestPreResignRequestExpiredCreds(t *testing.T) {
	...
	SignSDKRequest(r)
	querySig := r.HTTPRequest.URL.Query().Get("X-Aws-Signature")
	signedHeaders := r.HTTPRequest.URL.Query().Get("X-Aws-SignedHeaders")
	...
	assert.NotEqual(t, querySig, r.HTTPRequest.URL.Query().Get("X-Aws-Signature"))
	resignedHeaders := r.HTTPRequest.URL.Query().Get("X-Aws-SignedHeaders")
	assert.Equal(t, signedHeaders, resignedHeaders)
	assert.NotContains(t, signedHeaders, "x-aws-signedHeaders")
	assert.NotEqual(t, origSignedAt, r.LastSignedAt)
}
...
func BenchmarkStripExcessSpaces(b *testing.B) {
	vals := []string{
		`AWS4-HMAC-SHA256 Credential=AKIDFAKEIDFAKEID/20160628/us-west-2/s3/aws4_request, SignedHeaders=host;x-aws-date, Signature=1234567890abcdef1234567890abcdef1234567890abcdef`,
		`123   321   123   321`,
		`   123   321   123   321   `,
	}
