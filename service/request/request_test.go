...
func TestRequestRecoverRetry5xx(t *testing.T) {
	...
	s := awstesting.NewClient(service.NewConfig().WithMaxRetries(10))
...
func TestRequestExhaustRetries(t *testing.T) {
	...
	s := awstesting.NewClient(service.NewConfig().WithSleepDelay(sleepDelay))
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
	s.Handlers.Send.Clear() // mock sending
...
func TestRequestRecoverExpiredCreds(t *testing.T) {
	...
	s := awstesting.NewClient(&service.Config{MaxRetries: service.Int(10), Credentials: credentials.NewStaticCredentials("AKID", "SECRET", "")})
	s.Handlers.Validate.Clear()
	s.Handlers.Unmarshal.PushBack(unmarshal)
	s.Handlers.UnmarshalError.PushBack(unmarshalError)
...
func TestRequestUserAgent(t *testing.T) {
	s := awstesting.NewClient(&service.Config{Region: service.String("us-east-1")})
	//	s.Handlers.Validate.Clear()
	...
	expectUA := fmt.Sprintf("foo/bar %s/%s (%s; %s; %s)",
		service.SDKName, service.SDKVersion, runtime.Version(), runtime.GOOS, runtime.GOARCH)
