...
func TestValueAtPathSuccess(t *testing.T) {
	...
	for i, c := range testCases {
		v, err := ValuesAtPath(c.data, c.path)
...
func TestSetValueAtPathSuccess(t *testing.T) {
	var s Struct
	SetValueAtPath(&s, "C", "test1")
	SetValueAtPath(&s, "B.B.C", "test2")
	SetValueAtPath(&s, "B.D.C", "test3")
	assert.Equal(t, "test1", s.C)
	assert.Equal(t, "test2", s.B.B.C)
	assert.Equal(t, "test3", s.B.D.C)

	SetValueAtPath(&s, "B.*.C", "test0")
	assert.Equal(t, "test0", s.B.B.C)
	assert.Equal(t, "test0", s.B.D.C)

	var s2 Struct
	SetValueAtPath(&s2, "b.b.c", "test0")
	assert.Equal(t, "test0", s2.B.B.C)
	SetValueAtPath(&s2, "A", []Struct{{}})
	assert.Equal(t, []Struct{{}}, s2.A)

	str := "foo"

	s3 := Struct{}
	SetValueAtPath(&s3, "b.b.c", str)
	assert.Equal(t, "foo", s3.B.B.C)

	s3 = Struct{B: &Struct{B: &Struct{C: str}}}
	SetValueAtPath(&s3, "b.b.c", nil)
	...
	SetValueAtPath(&s4, "Name", str)
	assert.Equal(t, str, *s4.Name)

	s4 = struct{ Name *string }{}
	SetValueAtPath(&s4, "Name", nil)
