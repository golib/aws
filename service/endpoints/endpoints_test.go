...
func TestGenericEndpoint(t *testing.T) {
	...
	ep, sr := EndpointForRegion(name, region, false, false)
...
func TestDualStackEndpoint(t *testing.T) {
	ep, sr := EndpointForRegion("s3", "mock-region-1", false, true)
	assert.Equal(t, "https://s3.dualstack.mock-region-1.amazonaws.com", ep)
	assert.Equal(t, "", sr)

	ep, sr = EndpointForRegion("mock-svc", "mock-region-1", false, true)
	assert.Equal(t, "", ep)
	assert.Equal(t, "", sr)

	ep, sr = EndpointForRegion("s3", "mock-region-1", false, false)
...
func TestEC2MetadataEndpoints(t *testing.T) {
	...
	for _, region := range regions {
		ep, sr := EndpointForRegion("ec2metadata", region, false, false)
