...
func TestNewDefaultSession(t *testing.T) {
	...
	s := New(&service.Config{Region: service.String("region")})
	...
	assert.Equal(t, service.LogOff, *s.Config.LogLevel)
}
...
func TestNew_WithCustomCreds(t *testing.T) {
	...
	s := New(&service.Config{Credentials: customCreds})
...
func TestSessionCopy(t *testing.T) {
	...
	newSess := s.Copy(&service.Config{Region: service.String("new_region")})
...
func TestSessionClientConfig(t *testing.T) {
	s, err := NewSession(&service.Config{Region: service.String("orig_region")})
	assert.NoError(t, err)

	cfg := s.ClientConfig("s3", &service.Config{Region: service.String("us-west-2")})
