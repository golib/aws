...
func New(cfgs ...*service.Config) *Session {
...
func NewSession(cfgs ...*service.Config) (*Session, error) {
	envCfg := loadEnvConfig()
...
//     sess, err := session.NewSessionWithOptions(session.Options{
//          Config: service.Config{Region: service.String("us-east-1")},
//          Profile: "profile_name",
//     })
//
...
func oldNewSession(cfgs ...*service.Config) *Session {
	cfg := defaults.Config()
...
func newSession(envCfg envConfig, cfgs ...*service.Config) (*Session, error) {
	cfg := defaults.Config()
	handlers := defaults.Handlers()

	// Get a merged version of the user provided config to determine if
	// credentials were.
	userCfg := &service.Config{}
...
func mergeConfigSrcs(cfg, userCfg *service.Config, envCfg envConfig, sharedCfg sharedConfig, handlers request.Handlers) {
	// Merge in user provided configuration
	cfg.MergeIn(userCfg)

	// Region if not already set by user
	if len(service.StringValue(cfg.Region)) == 0 {
...
func initHandlers(s *Session) {
	...
	if !service.BoolValue(s.Config.DisableParamValidation) {
...
//     sess.Copy(&service.Config{Region: service.String("us-west-2")})
func (s *Session) Copy(cfgs ...*service.Config) *Session {
	newSession := &Session{
...
func (s *Session) ClientConfig(serviceName string, cfgs ...*service.Config) client.Config {
	s = s.Copy(cfgs...)
	endpoint, signingRegion := endpoints.NormalizeEndpoint(
		service.StringValue(s.Config.Endpoint),
		serviceName,
		service.StringValue(s.Config.Region),
		service.BoolValue(s.Config.DisableSSL),
		service.BoolValue(s.Config.UseDualStack),
	)
